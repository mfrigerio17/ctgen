...
class Generator:
    ...
    def _loadLuaCode(self, sourceFilePath):
        luaCodeSrc = open_utf8_reading( sourceFilePath )
    ...
    def generate(self, ctModelMetadata, allMatricesMetadata, outputDirectory):
        ok, text = self.generate_code(ctModelMetadata, allMatricesMetadata)
        if not ok:
            logger.error("Failed to generate file: %s", text)
        else:
            with open_utf8_writing(outputDirectory / "generated.txt") as dst:
                dst.write(text)
